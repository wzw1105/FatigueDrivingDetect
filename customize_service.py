...
def get_model_base_path(model_dir):
    ...
    if model_dir is None:
        if not os.path.exists(model_base_path):
            model_base_path = resolve(os.path.join("../../../../../Downloads", "models"))
    else:
...
class fatigue_driving_detection(PTServingBaseService): #PTServingBaseService
    ...
    def _inference(self, data):
        result = {"result": {"duration": 6000, "drowsy": []}}
        # period_start = 1000
        # period_end = 2000
        # period_type = 4
        # result["result"]["drowsy"].append({"periods": [period_start, period_end], "category": period_type})
        # print(result)
        # exit(1)
                ...
                if frame is not None:
                    ...
                    for cls in [3, 4, 2, 1]:
                        ...
                        if find_final_result:
                            if cls is not period_type:
                                continue
                            if self.previous_frames_result[store_index][cls] == 1:
                                period_end = store_index
                                continue
                            if store_index - period_end > 3 * self.scan_every:
                                result["result"]["drowsy"].append(
                                    {"periods": [period_start * 1000 // self.fps, period_end * 1000 // self.fps],
                                     "category": period_type})
                                period_type = 0
                                period_start = store_index
                                find_maybe_result, find_final_result = False, False
                                find_result_time = -1

                        if store_index < period_start + self.frame_3s:
                            continue
                        ...
                        if find_maybe_result:
                            if store_index - find_result_time <= (self.fps / 2):
                                if cur_frame_3s_sum > 0.85 * self.frame_3s and self.cls_priority[cls] > self.cls_priority[period_type]:
                        ...
                        if cur_frame_3s_sum > 0.85 * self.frame_3s and find_maybe_result is False:
                            ...
                            period_type = cls
                            period_start = 0 if store_index - self.frame_3s <= 0 else store_index - self.frame_3s
                            while self.previous_frames_result[period_start][period_type] == 0:
                                period_start += 1
                            period_end = store_index
